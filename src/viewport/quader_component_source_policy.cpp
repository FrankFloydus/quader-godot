...
bool contains_object(std::span<const ObjectId> objects, ObjectId id) {
	return std::find(objects.begin(), objects.end(), id) != objects.end();
}

void append_unique(std::vector<ObjectId> &objects, ObjectId id) {
...
ObjectId primary_component_source_object(
...
std::vector<ObjectId> component_source_wire_objects(
		std::span<const ComponentSourceObjectState> objects, SelectionMode mode,
		ObjectId candidate) {
...
std::vector<ObjectId> component_vertex_handle_objects(
		std::span<const ComponentSourceObjectState> objects, SelectionMode mode,
		ObjectId candidate) {
	std::vector<ObjectId> result;
	if (mode != SelectionMode::Vertex) {
...
ObjectId component_hover_candidate(
...
std::vector<ObjectId> component_source_wire_objects_for_hover(
		std::span<const ComponentSourceObjectState> objects, SelectionMode mode,
